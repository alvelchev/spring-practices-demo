...
class ReadingJsonControllerTest {
    ...
    void testThat_getCountries_returnsResult() throws Exception {
        ...
        // Act
        var mvcResult = mockMvc.perform(get(WebPath.API_VERSION_1 + WebPath.PATH_COUNTRIES)
                        .param("name", TEST_PARAM)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();

        var actualResult = objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
                new TypeReference<Set<CountryDTO>>() {});
    ...
    void testThat_getCountriesInFrenchOverseasRegion_returnsResult() throws Exception {
        ...
        // Act
        var mvcResult = mockMvc.perform(get(WebPath.API_VERSION_1 + WebPath.PATH_COUNTRIES_REGION_TYPE, TEST_CE_REGION_TYPE)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
