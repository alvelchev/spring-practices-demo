...
class ReadingJsonServiceTest {
    ...
    void testThat_getCountries_returnsResult() {
        // Arrange
        when(mockCountryStorage.getCountriesByCountryNameStartsWith(or(isNull(), anyString()))).thenReturn(expectedResult);
    ...
    void testThat_getCountriesByRegionType_whenRegionIsNotSupported() {
        ...
        assertEquals(String.format("Not supported region %d", TEST_NON_SUPPORTED_REGION_TYPE), exception.getMessage());
    }
}
