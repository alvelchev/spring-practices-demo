...
class CriteriaBuilderExampleServiceTest {
    ...
    void testThat_getUsersByListOfExcludeUsernames_returnsTheExpectedResult() {
        // Arrange
        when(mockUserRepository.getUsers(anyList(), anyList(), anyList(), anyString(), any(Pageable.class))).thenReturn(
                new PageImpl<>(users));

        // Act
        var actualResult = underTest.getUsers(List.of(), List.of(TEST_422_USERNAME), List.of(), Strings.EMPTY, mockPageable);

        // Assert
        verify(mockUserRepository).getUsers(List.of(), List.of(TEST_422_USERNAME), List.of(), Strings.EMPTY, mockPageable);
    ...
    void testThat_getUsersByListOfExcludeUsernamesAndProvidedLdapGroups_returnsTheExpectedResult() {
        ...
        // Act
        var actualResult = underTest.getUsers(List.of(), List.of(TEST_422_USERNAME), ldapGroupsMock, Strings.EMPTY,
                mockPageable);

        // Assert
        verify(mockUserRepository).getUsers(List.of(), List.of(TEST_422_USERNAME), ldapGroupsMock, Strings.EMPTY,
                mockPageable);
        assertEquals(expectedResult, actualResult.getContent());
    }
    ...
    void testThat_getUsersByListOfExcludeUsernamesAndSearchParameter_returnsTheExpectedResult() {
        ...
        // Act
        var actualResult = underTest.getUsers(List.of(), List.of(TEST_422_USERNAME), List.of(), TEST_422_USERNAME,
                mockPageable);

        // Assert
        verify(mockUserRepository).getUsers(List.of(), List.of(TEST_422_USERNAME), List.of(), TEST_422_USERNAME,
                mockPageable);
        assertEquals(expectedResult, actualResult.getContent());
    }
}
